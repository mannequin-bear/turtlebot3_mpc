...
class MPC(Node):
    ...
    def visualize_path(self):
        ...
        # Iterate over the horizon N+1
        # prev_sol_X shape is (3, N+1) -> [x, y, theta]
        for i in range(self.prev_sol_X.shape[1]):
            pose = PoseStamped()
            pose.header = msg.header
            
            # Position
            pose.pose.position.x = float(self.prev_sol_X[0, i])
            pose.pose.position.y = float(self.prev_sol_X[1, i])
            pose.pose.position.z = 0.0

            # Orientation (Yaw to Quaternion)
            # Since robot is 2D, we rotate around Z axis
            yaw = float(self.prev_sol_X[2, i])
            # Simple conversion: q_w = cos(yaw/2), q_z = sin(yaw/2)
    ...
    def control_loop(self):
        ...
        error = self.get_parameter('error').value

        start_time = self.get_clock().now()
        optimal_u = np.array([0.0, 0.0])
        if(np.any(abs(self.goal_state - self.current_state) >= np.array([error, error, 0.2]))):
